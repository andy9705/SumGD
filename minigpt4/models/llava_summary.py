...
class LLaVa(BaseModel):
    ...
    def generate(
        ...
        with self.maybe_autocast():
            ...
            input_ids_llm = torch.cat([bos, tokens_before, tokens_after], dim=1)
            ...
            output_ids,jsd_list,kl_list,reverse_kl_list = self.llama_model.generate( ##jsd_list,top_k_list
                input_ids=input_ids,
        ...
        output_token_ids=output_ids[:, input_token_len:].tolist()[0]
        

        output_tokens = self.llama_tokenizer.convert_ids_to_tokens(output_token_ids)
        ...
        jsd_output_tokens = []
        kl_output_tokens = []
        reverse_kl_output_tokens = []
        if len(jsd_list) == len(kl_list) == len(reverse_kl_list) == len(output_tokens):
            pass
        else:
        ...
        for _idx in range(len(output_tokens)):
            kl_output_tokens.append([output_tokens[_idx],kl_list[_idx]])

        for _idx in range(len(output_tokens)):
            reverse_kl_output_tokens.append([output_tokens[_idx],reverse_kl_list[_idx]])
        
        return output_text, jsd_output_tokens,kl_output_tokens,reverse_kl_output_tokens
