...
class MiniGPT4(Blip2Base):
    ...
    def generate(
        ...
        #### llm 시작 prompt
        llM_instruction = '###Human: Please describe this image in detail. ###Assistant:'
        llm_inputs=self.llama_tokenizer(
                llM_instruction, return_tensors="pt", add_special_tokens=False).to(self.device)
        llm_inputs_embeds=self.embed_tokens(llm_inputs.input_ids)
        llm_attention_mask=llm_inputs.attention_mask
        
        batch_size = 1
        llm_bos = torch.ones([1, 1],
                        dtype=torch.int64,
                        device=llm_inputs_embeds.device) * self.llama_tokenizer.bos_token_id
        llm_bos_embeds = self.embed_tokens(llm_bos)
        llm_atts_bos = llm_attention_mask[:, :1]
        llm_inputs_embeds = torch.cat([llm_bos_embeds, llm_inputs_embeds], dim=1)
        llm_attention_mask = torch.cat([llm_atts_bos, llm_attention_mask], dim=1)
        ...
        with self.maybe_autocast():
            ...
            output_ids,jsd_list = self.llama_model.generate(
                inputs_embeds=inputs_embeds,
                attention_mask=attention_mask,
                inputs_embeds_llm = llm_inputs_embeds,
                attention_mask_llm = llm_attention_mask,
        ...
        #print(outputs)
        outputs= outputs[1:]
        output_tokens = self.llama_tokenizer.convert_ids_to_tokens(outputs)
        #print(jsd_list)
        #print(output_tokens)
        #print(len(jsd_list))
        ...
        return output_text,jsd_output_tokens
