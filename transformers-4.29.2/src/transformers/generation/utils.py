...
def token_distribution_jsd(lvlm, llm):
    # Compute the probability distributions using softmax
    #print(llm.squeeze(),lvlm.squeeze())
    prob1 = F.softmax(llm.squeeze(), dim=0)
    prob2 = F.softmax(lvlm.squeeze(), dim=0)
    #print(prob1)
    #print(prob2)
    # Compute the mean of the two probability distributions
    m = 0.5 * (prob1 + prob2)

    # Compute the KL-divergence for each probability distribution relative to the mean
    kl_div_1 = F.kl_div(m.log(), prob1, reduction='sum') ##sum
    kl_div_2 = F.kl_div(m.log(), prob2, reduction='sum') ##sum
    #print(kl_div_1,kl_div_2)
    # Compute the Jensen-Shannon divergence
    jsd = 0.5 * (kl_div_1 + kl_div_2)
    #print(jsd)
    return jsd.item()
...
class GenerationMixin:
    ...
    def generate(
        ...
        summary_guided_decoding: Optional[bool] = None,
        ...
        **kwargs,
        ...
        if is_greedy_gen_mode and opera_decoding:
        ...
        elif is_greedy_gen_mode and summary_guided_decoding:
            if generation_config.num_return_sequences > 1:
                raise ValueError(
                    "num_return_sequences has to be 1 when doing greedy search, "
                    f"but is {generation_config.num_return_sequences}."
            ...
            # 11. run greedy search
            return self.summary_guided_decoding(
                input_ids,
                logits_processor=logits_processor,
                stopping_criteria=stopping_criteria,
                pad_token_id=generation_config.pad_token_id,
